 ...
 * @param Network a network class which supports
 * 			send,
 * 			receive
 *
 ...
 * 		Karthik added this
 * 			Timer() => constructor
 * 			Timer(int timeoutValue)  => constructor with timeout value as input
 * 			Timer.left_ms()  => time left to expire
 * 			Timer.expired()  => boolean stating timer expired or not
 */
...
MQTTSN::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::Client(Network& network, unsigned int command_timeout_ms)
: ipstack(network), packetid(),duration(0),cleansession(false)
{
    last_sent = Timer();
    last_received = Timer();
